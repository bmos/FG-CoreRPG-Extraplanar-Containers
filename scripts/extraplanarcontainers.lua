...
tExtraplanarContainers = {
    'knapsack of halflingkind', 'quiver of ehlonna', 'efficient quiver',
    'handy haversack', 'portable hole', 'extraplanar', 'weightless',
...
tContainers = {
    'container', 'backpack', 'satchel', 'quiver', 'chest', 'purse', 'pouch',
    'sack', 'bag', 'box'
}
...
function isAnyContainer(sItemName)
    if sItemName and sItemName ~= '' then
        ...
        return isContainer(sItemNameLower, tExtraplanarContainers) or
                   isContainer(sItemNameLower, tContainers)
    end
end
...
local function build_containers(node_inventory)
    ...
    for _, node_item in pairs(DB.getChildren(node_inventory)) do
        local string_item_name =
            string.lower(DB.getValue(node_item, 'name', ''))
        local number_maxweight = DB.getValue(node_item, 'capacityweight', 0);

        local bool_extraplanar = isContainer(string_item_name,
                                             tExtraplanarContainers)
...
local function measure_contents(node_inventory, table_containers_mundane,
                                table_containers_extraplanar)
    ...
    for _, node_item in pairs(DB.getChildren(node_inventory)) do
        ...
        if state_item_carried ~= 0 then
            ...
            local string_item_location = string.lower(
                                             DB.getValue(node_item, 'location',
                                                         ''))

            local bIsInExtraplanar = isContainer(string_item_location,
                                                 tExtraplanarContainers)
            local bIsInContainer =
                isContainer(string_item_location, tContainers)
                ...
                if table_containers_extraplanar[string_item_location] then
                    ...
                    if OptionsManager.isOption('ITEM_VOLUME', 'on') then
                        table_containers_extraplanar[string_item_location]['nTotalVolume'] =
                            (table_containers_extraplanar[string_item_location]['nTotalVolume'] +
                                (number_item_count *
                                    DB.getValue(node_item, 'volume', 0)))
                        if table_containers_extraplanar[string_item_location]['nMaxLength'] <
                            DB.getValue(node_item, 'length', 0) then
                            table_containers_extraplanar[string_item_location]['bTooBig'] =
                                1
                        end
                        if table_containers_extraplanar[string_item_location]['nMaxWidth'] <
                            DB.getValue(node_item, 'width', 0) then
                            table_containers_extraplanar[string_item_location]['bTooBig'] =
                                1
                        end
                        if table_containers_extraplanar[string_item_location]['nMaxDepth'] <
                            DB.getValue(node_item, 'depth', 0) then
                            table_containers_extraplanar[string_item_location]['bTooBig'] =
                                1
                        end
                    end
                end
            elseif state_item_carried ~= 2 and
                (bIsInContainer and not bIsInExtraplanar) then
                if table_containers_mundane[string_item_location] then
                    ...
                    if OptionsManager.isOption('ITEM_VOLUME', 'on') then
                        table_containers_mundane[string_item_location]['nTotalVolume'] =
                            (table_containers_mundane[string_item_location]['nTotalVolume'] +
                                (number_item_count *
                                    DB.getValue(node_item, 'volume', 0)))
                        if table_containers_mundane[string_item_location]['nMaxLength'] <
                            DB.getValue(node_item, 'length', 0) then
                            table_containers_mundane[string_item_location]['bTooBig'] =
                                1
                        end
                        if table_containers_mundane[string_item_location]['nMaxWidth'] <
                            DB.getValue(node_item, 'width', 0) then
                            table_containers_mundane[string_item_location]['bTooBig'] =
                                1
                        end
                        if table_containers_mundane[string_item_location]['nMaxDepth'] <
                            DB.getValue(node_item, 'depth', 0) then
                            table_containers_mundane[string_item_location]['bTooBig'] =
                                1
                        end
                    ...
                    local string_item_location_location = string.lower(
                                                              DB.getValue(
                                                                  table_containers_mundane[string_item_location]['nodeItem'],
                                                                  'location', ''))
                ...
                number_total_weight = number_total_weight +
                                          (number_item_count *
                                              number_item_weight)
            end
        end
...
local function write_contents_to_containers(node_inventory, table_containers,
                                            string_error)
    local rActor = ActorManager.resolveActor(node_inventory.getParent())
    for _, table_container in pairs(table_containers) do
        DB.setValue(table_container['nodeItem'], 'extraplanarcontents',
                    'number', table_container['nTotalWeight'])
        DB.setValue(table_container['nodeItem'], 'contentsvolume', 'number',
                    table_container['nTotalVolume'])
        local string_item_name = DB.getValue(table_container['nodeItem'],
                                             'name', 'extraplanar container')
        ...
        if table_container['nMaxWeight'] > 0 then
            if (table_container['nTotalWeight'] > table_container['nMaxWeight']) then
                ...
                if not table_container['nodeItem'].getChild('announcedW') then
                    DB.setValue(table_container['nodeItem'], 'announcedW',
                                'number', 1)
                    ChatManager.Message(string.format(
                                            Interface.getString(string_error),
                                            string_item_name, 'weight'), true,
                                        rActor)
                end
            else
                if table_container['nodeItem'].getChild('announcedW') then
                    table_container['nodeItem'].getChild('announcedW').delete()
                end
            end
        end

        -- check volume of contents and announce if excessive
        if OptionsManager.isOption('ITEM_VOLUME', 'on') and
            table_container['nMaxVolume'] > 0 then
            if table_container['bTooBig'] == 1 then
                if not table_container['nodeItem'].getChild('announcedV') then
                    DB.setValue(table_container['nodeItem'], 'announcedV',
                                'number', 1)
                    ChatManager.Message(string.format(
                                            Interface.getString(string_error),
                                            string_item_name,
                                            'maximum dimension'), true, rActor)
                end
            elseif table_container['nTotalVolume'] >
                table_container['nMaxVolume'] then
                if not table_container['nodeItem'].getChild('announcedV') then
                    DB.setValue(table_container['nodeItem'], 'announcedV',
                                'number', 1)
                    ChatManager.Message(string.format(
                                            Interface.getString(string_error),
                                            string_item_name, 'volume'), true,
                                        rActor)
                end
            else
                if table_container['nodeItem'].getChild('announcedV') then
                    table_container['nodeItem'].getChild('announcedV').delete()
                end
            end
        end
    end
...
local function updateContainers(node_inventory)
    ...
    local table_containers_mundane, table_containers_extraplanar =
        build_containers(node_inventory)

    -- this will contain a running total of all items carried by the character
    local number_total = measure_contents(node_inventory,
                                          table_containers_mundane,
                                          table_containers_extraplanar) or 0

    -- writes container subtotals to database and handles chat messages
    write_contents_to_containers(node_inventory, table_containers_mundane,
                                 "item_overfull")
    write_contents_to_containers(node_inventory, table_containers_extraplanar,
                                 "item_self_destruct")

    number_total = number_total +
                       CharEncumbranceManager.calcDefaultCurrencyEncumbrance(
                           node_char);
    ...
    DB.setValue(node_char, CharEncumbranceManager.getEncumbranceField(),
                'number', number_rounded_total)
end
...
function onInit()
    OptionsManager.registerOption2('ITEM_VOLUME', false, 'option_header_game',
                                   'opt_lab_item_volume', 'option_entry_cycler',
                                   {
    ...
    if Session.IsHost then
        for _, sItemListNodeName in pairs(
                                        ItemManager.getInventoryPaths(
                                            'charsheet')) do
            local sItemList = 'charsheet.*.' .. sItemListNodeName;
            DB.addHandler(DB.getPath(sItemList .. '.*.capacityweight'),
                          'onUpdate', onItemUpdate);
            DB.addHandler(DB.getPath(sItemList .. '.*.location'), 'onUpdate',
                          onItemUpdate);
            DB.addHandler(DB.getPath(sItemList .. '.*.name'), 'onUpdate',
                          onItemUpdate);
            DB.addHandler(DB.getPath(sItemList .. '.*'), 'onChildDeleted',
                          onItemDeleted);

            -- external size fields
            DB.addHandler(DB.getPath(sItemList .. '.*.length'), 'onUpdate',
                          onItemUpdate);
            DB.addHandler(DB.getPath(sItemList .. '.*.width'), 'onUpdate',
                          onItemUpdate);
            DB.addHandler(DB.getPath(sItemList .. '.*.depth'), 'onUpdate',
                          onItemUpdate);

            -- internal size fields
            DB.addHandler(DB.getPath(sItemList .. '.*.internal_length'),
                          'onUpdate', onItemUpdate);
            DB.addHandler(DB.getPath(sItemList .. '.*.internal_width'),
                          'onUpdate', onItemUpdate);
            DB.addHandler(DB.getPath(sItemList .. '.*.internal_depth'),
                          'onUpdate', onItemUpdate);
        end
    end
end
